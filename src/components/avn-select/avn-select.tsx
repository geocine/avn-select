...
export class AvnSelect {
  ...
  onClick(e: any) {
    const path =
      e.path || (e.composedPath && e.composedPath()) || this.propagationPath(e);
    let nativeElement = this.host.shadowRoot;
    let elementToggle = nativeElement !== path[0];
    let containedToggle = !nativeElement.contains(path[0])
      ? false
      : !this.toggle;
  ...
  scrollToItem(index, down = true) {
    this.dropdownItems = this.host.shadowRoot.querySelector(
      '.avn-select-items'
    );
      ...
      switch (index) {
        ...
        default:
          if (!this.isScrolledIntoView(index)) {
            ...
            this.dropdownItems.scrollTop = down
  ...
  render() {
    ...
    let clearButton = (
      <a class="avn-select-clear" onClick={this.resetSelectedValue.bind(this)}>
    ...
    let itemList = this.optionList.map((item, i) => {
      return (
        <div
          class={`avn-select-content-item item-${i} ${
    ...
    let list = (
      <div class="avn-select-content">
        <div class="avn-select-items">{itemList}</div>
      </div>
    );
    ...
    return (
      <div class={`avn-select ${this.disabled ? 'state-disabled' : ''}`}>
        ...
        <input
          class={`avn-select-input ${this.disabled ? 'state-disabled' : ''}`}
          value={this.value.label}
          type="text"
          tabindex="-1"
          placeholder={this.placeholder}
        />
        <div class="avn-select-overlay" />
        {/* </div>​ */}
        {clear ? clearButton : ''}
